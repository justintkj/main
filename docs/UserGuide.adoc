...
Note : While Jo did the numsearched fields for sort, i refractored to use streams to
reduce excessive "for loops".
...
* Valid Arguments are: `name`, `number`, vaddress`, `email`, `remark`, `birthday`, `favourite`, `numtimessearched`.
...
* *Sort* `sort name` + `sort number` + `sort address` + `sort email` + `sort remark` + `sort birthday` + `sort favourite` + `sort numtimessearched`
